...
def s_pure_exchang() :
    ...
    pure_ftor1 = merge['turnover_rate_' + t] >= pure_bond['换手率下限']
    pure_ftor2 = merge['turnover_rate_' + t_b1] >= pure_bond['换手率下限']
    pure_ftor3 = merge['turnover_rate_' + t_b2] >= pure_bond['换手率下限']
    pure_ftor4 = merge['turnover_rate_' + t_b3] >= pure_bond['换手率下限']

    pure_ftor5 = merge['turnover_rate_' + t] <= pure_bond['换手率上限']
    pure_ftor6 = merge['turnover_rate_' + t_b1] <= pure_bond['换手率上限']
    pure_ftor7 = merge['turnover_rate_' + t_b2] <= pure_bond['换手率上限']
    pure_ftor8 = merge['turnover_rate_' + t_b3] <= pure_bond['换手率上限']
...
def s_tor_factor() :
    ...
    ftor5 = merge['turnover_rate_' + t] >= turnover_rarion_bound['今日换手率下限']
    ftor6 = merge['turnover_rate_' + t_b1] >= turnover_rarion_bound["昨日换手率"]
...
def s_v_style() :
    ...
    p_ftor11 = merge['turnover_rate_' + t_b1] >= v_style_bond['换手率下限']
...
def s_increase_daily():
    ...
    for ts_code in s_increase_daily['ts_code'] :
        print("正在计算上升形态策略:", ts_code)
        df = ts.pro_bar(ts_code=ts_code, adj='qfq', start_date='20190700', end_date=t, ma=[5])[
            ['ts_code', 'close', 'ma5']]
        ma_filter = ma_filter.append(df.head(1))  # 取第一行为今天的日均线数据
    print("上升形态策略完毕！！！")
    ...
    s_increase_daily_temp = pd.merge(ma_filter, s_increase_daily, on='ts_code', sort=False,
                              left_index=False, right_index=False, how='left')

    s_increase_daily_selected = s_increase_daily_temp.copy()
    s_increase_daily_selected = s_increase_daily_selected[['name', 'ts_code', 'area', 'industry', 'close_x', 'close_y',
                                             'trade_date', 'pct_chg_' + t, 'pct_chg_' + t_n1, 'pe', 'pe_ttm']]
...
def s_doji_line() :
    ...
    b_case2 = merge['turnover_rate_' + t_b1] >= doji_line_bond['换手率下限']
...
def s_cross_all_average() :
    ...
    b_case6 = merge['turnover_rate_' + t] >= cross_all_average_bond['换手率下限']
...
换手率放大3倍
'''
def s_turnoverrate_times():
    s_turnoverrate_times_bond = {
        '市盈率(pe)上限' : 80,
        '市盈率(pe)下限' : 15,
        '市盈率(pe_ttm)下限' : 15,
        '换手率下限' : 4.0,
    ...
    p_ftor3 = merge.pe <= s_turnoverrate_times_bond['市盈率(pe)上限']
    p_ftor4 = (merge['turnover_rate_' + t] /merge['turnover_rate_' + t_b1]) >= s_turnoverrate_times_bond['换手率放大倍数']

    s_turnoverrate_times = merge[p_ftor1 & p_ftor4]

    ma_filter = pd.DataFrame()
    for ts_code in s_turnoverrate_times['ts_code'] :
        print("正在计算换手率放大策略:", ts_code)
        df = ts.pro_bar(ts_code=ts_code, adj='qfq', start_date='20190101', end_date=t, ma=[5])[
            ['ts_code', 'close', 'ma5']]
        ma_filter = ma_filter.append(df.head(1))  # 取第一行为今天的日均线数据
    print("换手率放大策略计算完毕！！！")
    ...
    s_turnoverrate_times_temp = pd.merge(ma_filter, s_turnoverrate_times, on='ts_code', sort=False,
                              left_index=False, right_index=False, how='left')

    s_turnoverrate_times_selected = s_turnoverrate_times_temp.copy()
    s_turnoverrate_times_selected = s_turnoverrate_times_selected[['name', 'ts_code', 'area', 'industry', 'close_x', 'close_y',
                                             'trade_date', 'pct_chg_' + t, 'pct_chg_' + t_n1, 'pe', 'pe_ttm']]

    s_turnoverrate_times_selected.sort_values(by='pct_chg_' + t_n1, inplace=True, ascending=False)  # 重新排序
    s_turnoverrate_times_selected.index = range(len(s_turnoverrate_times_selected))

    return s_turnoverrate_times_selected
